...
CREATE POLICY products_public_read ON public.products FOR SELECT TO anon, authenticated USING (is_active = true AND is_hidden = false);
...
CREATE POLICY variants_public_read ON public.product_variants FOR SELECT TO anon, authenticated USING (is_active = true);
...
BEGIN
  ...
  ON CONFLICT (date_key) DO UPDATE
    SET last_sequence = public.order_sequence.last_sequence + 1, updated_at = now()
